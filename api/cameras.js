...
module.exports = async (req, res) => {
  if (req.method === 'GET') {
    ...
    try {
      ...
      const result = accessibleIds === null
        ? await db.query(`${baseSelect} WHERE c.organization_id = $1 ORDER BY c.id`, [auth.organizationId])
        : await db.query(`${baseSelect} WHERE c.organization_id = $1 AND c.id = ANY($2::varchar[]) ORDER BY c.id`,
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const existing = await db.query('SELECT organization_id, media_node_id FROM cameras WHERE id = $1', [id]);
