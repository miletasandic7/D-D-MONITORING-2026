...
export default function Dashboard() {
  ...
  const saveCamera = async () => {
    ...
    const payload = {
      ...newCamera,
      lat: newCamera.lat === '' || newCamera.lat == null ? undefined : Number(newCamera.lat),
      lng: newCamera.lng === '' || newCamera.lng == null ? undefined : Number(newCamera.lng),
    };
    try {
      await api.post('/cameras', payload);
      setCameras((prev) => [...(prev || []), payload]);
      setWizardDone(true);
      setWizardStep(3);
    } catch (err) {
      setWizardDone(false);
  ...
  const submitAddCamera = async (e) => {
    ...
    const newCam = {
      ...
      lat: addCamForm.lat ? Number(addCamForm.lat) : undefined,
      lng: addCamForm.lng ? Number(addCamForm.lng) : undefined,
      enabled: true,
